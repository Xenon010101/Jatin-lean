    ...
    match command {
            ...
            if clear {
                ...
                if ctx.json {
                    ...
                    println!(
                        "  {} Cache file: {}",
                        style("◉").dim(),
                        style(crate::cache::ScanCache::cache_path(&nm_path).display()).dim()
                    );
                    println!();
                }
            }
        ...
        AnalyzeCommands::Engine {
            ...
            if !ctx.json {
                println!(
                    "  {} {}",
                    style("Adaptive CPU-GPU Execution Engine").cyan().bold(),
                    style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").dim()
                );
            }
            if compare {
            ...
            let hw = if grace_hopper {
                if !ctx.json {
                    println!(
                        "  {} Simulating NVIDIA Grace Hopper GH200 environment",
                        style("🖥️").yellow()
                    );
                }
            ...
            if analyze || bench {
                let workloads = vec![
                    ...
                    WorkloadProfile {
                        ...
                        matrix_heavy: false,
                        branch_divergent: false,
                        memory_required_bytes: 2 * 1024 * 1024,
                ...
                if ctx.json {
                    ...
                    for w in &workloads {
                        ...
                        println!(
                            ...
                            result.target.label(),
                            style("").dim(),
                            style(format!("{:.0}", result.estimated_speedup))
                                .green()
                                .bold(),
            ...
            if !analyze && !bench && !compare {
                if !ctx.json {
                    print_engine_report(&engine.stats, &engine.hardware);
                    println!(
                        "  {} Use {} for workload routing analysis",
                        style("→").dim(),
                        style("jatin-lean analyze engine --analyze").yellow()
                    );
                    println!(
                        "  {} Use {} for Grace Hopper simulation",
                        style("→").dim(),
                        style("jatin-lean analyze engine --grace-hopper --analyze").yellow()
                    );
                    println!();
                }
            }
        ...
        AnalyzeCommands::Snapshots {
            ...
            if let Some(snapshot_id) = restore {
                ...
                if ctx.json {
                    ...
                    json_out.insert("result".to_string(), serde_json::json!({"success": true}));
                // Simplified
                } else {
        ...
        AnalyzeCommands::Undo => {
            ...
            if let Some(latest) = snapshots.first() {
                ...
                if ctx.json {
                    json_out.insert(
                        "error".to_string(),
                        serde_json::json!("No snapshots found to undo."),
                    );
                } else {
                    println!(
        ...
        AnalyzeCommands::Restore { snapshot_id } => {
            ...
            if ctx.json {
                json_out.insert(
                    "restored_snapshot_id".to_string(),
