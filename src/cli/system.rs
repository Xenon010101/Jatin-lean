    ...
    match command {
        SystemCommands::Optimize {
            ...
            if network {
                ...
                if ctx.json {
                    ...
                    println!(
                        "    TCP_NODELAY:      {}",
                        if tuning.tcp_nodelay { "✓" } else { "✗" }
                    );
                    println!(
                        "    TCP_QUICKACK:     {}",
                        if tuning.tcp_quickack { "✓" } else { "✗" }
                    );
                    println!(
                        "    SO_REUSEPORT:     {}",
                        if tuning.so_reuseport { "✓" } else { "✗" }
                    );
                    println!(
                        "    TCP_FASTOPEN:     {}",
                        if tuning.tcp_fastopen { "✓" } else { "✗" }
                    );
                    println!(
                        "    Recv buffer:      {} MB",
                        tuning.recv_buffer_size / 1024 / 1024
                    );
                    println!(
                        "    Send buffer:      {} MB",
                        tuning.send_buffer_size / 1024 / 1024
                    );
            ...
            if kernel {
                ...
                if ctx.json {
                    ...
                    println!();
                    println!(
                        "  {} Kernel Parameters (scan-optimized):",
                        style("🔧").yellow()
                    );
                    println!("    fs.file-max:        {}", tuning.fs_file_max);
                    println!(
                        "    inotify watches:    {}",
                        tuning.fs_inotify_max_user_watches
                    );
                    ...
                    println!(
                        "    THP:                {}",
                        if tuning.transparent_hugepages {
                            "enabled"
                        } else {
                            "disabled"
                        }
        ...
        SystemCommands::CpuCache {
            ...
            if !ctx.json {
                println!(
                    "  {} {}",
                    style("CPU Cache Optimization Engine").cyan().bold(),
                    style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").dim()
                );
            }
            ...
            if info {
                if ctx.json {
                    ...
                    println!(
                        ...
                        hierarchy.l1d_latency_ns
                    );
                    println!("    L1I:     {}KB", hierarchy.l1i_size / 1024);
                    println!(
                        "    L2:      {}KB ({:.0}ns)",
                        hierarchy.l2_size / 1024,
                        hierarchy.l2_latency_ns
                    );
                    println!(
                        "    L3:      {}MB ({:.0}ns)",
                        hierarchy.l3_size / 1024 / 1024,
                        hierarchy.l3_latency_ns
                    );
                    println!("    Main:    {:.0}ns", hierarchy.main_memory_ns);
                    println!(
                        "    Line:    {}B | TLB: {} entries",
                        hierarchy.cache_line_size, hierarchy.tlb_entries
                    );
                    println!(
                        "    Items/L1: {} (64B) | Items/L2: {} | Items/L3: {}",
                        hierarchy.l1_items(64),
                        hierarchy.l2_items(64),
                        hierarchy.l3_items(64)
                    );
                    println!();
                }
            }
            ...
            if bench {
                ...
                if !ctx.json {
                    println!(
                        "  {} Prefetch benchmark: {} items ({} KB working set)",
                        style("⚡").yellow(),
                        style(item_count).green().bold(),
                ...
                if ctx.json {
                    let speedup =
                        no_prefetch.as_nanos() as f64 / with_prefetch.as_nanos().max(1) as f64;
                    json_out.insert(
                        "benchmark".to_string(),
                        serde_json::json!({
                            "items": item_count,
                    ...
                    println!();
                    println!(
                        "  {} Without prefetch: {:.2} ms ({:.0} ns/item)",
                        style("▸").dim(),
                        no_prefetch.as_secs_f64() * 1000.0,
                        no_prefetch.as_nanos() as f64 / item_count as f64
                    );
                    println!(
                        "  {} With prefetch:    {:.2} ms ({:.0} ns/item)",
                        style("⚡").yellow(),
                        with_prefetch.as_secs_f64() * 1000.0,
                    ...
                    println!(
                        "  {} Speedup: {}x",
                        style("🚀").yellow(),
                        style(format!("{:.2}", speedup)).green().bold()
                    );
            ...
            if tlb {
                ...
                if ctx.json {
                    ...
                    println!();
                    println!(
                        "  {} TLB Analysis for {} KB working set:",
                        style("📊").yellow(),
                        working_set_kb
                    );
                    ...
                    println!(
                        "    TLB coverage (4KB):  {:.1}%",
                        info.tlb_coverage_standard
                    );
                    println!("    TLB coverage (2MB):  {:.1}%", info.tlb_coverage_huge);
                    println!(
                        "    Recommend hugepages: {}",
                        if info.recommend_huge_pages {
                            style("YES").green().bold()
                        } else {
                            style("no").dim()
                        }
            ...
            if !bench && !info && !tlb {
                if !ctx.json {
                    println!();
                    println!(
                        "  {} Use {} for cache hierarchy info",
                        style("→").dim(),
                        style("jatin-lean system cpu-cache --info").yellow()
                    );
                    println!(
                        "  {} Use {} for prefetch benchmark",
                        style("→").dim(),
